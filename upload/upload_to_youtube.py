"""
YouTube Upload Script - Velocity Telugu
Updated for 2025
...
def generate_video_metadata(category: str, num_phrases: int, phrases: list = None):
    """Generate Telugu title, description, and tags for the video"""

    title = f"Telugu Learning: {num_phrases} Essential {category} Phrases"

    description_lines = [
        f"Learn Telugu with Velocity Telugu!",
        f"",
        f"Category: {category}",
        f"",
        f"Master Telugu one phrase at a time! Today's {category} lesson:",
        f""
    ]
    ...
    if phrases:
        ...
        for i, phrase in enumerate(phrases[:5], 0):
            ...
            description_lines.append(f"   {phrase.get('telugu', '')}")
    ...
    description_lines.extend([
        ...
        f"Subscribe for daily Telugu lessons!",
        f"",
        f"Pronunciation Guide:",
        f"   The phonetic spelling in brackets helps you say it correctly!",
        f"",
        f"#LearnTelugu #TeluguLessons #TeluguForBeginners #LanguageLearning",
        f"#Telugu #Education #Tutorial #DailyTelugu #{category.replace(' ', '')}",
        f"#VelocityTelugu #TeluguPhrases #SpeakTelugu"
    ])
...
def upload_to_youtube(video_path, title, description, tags=None, category_id='22'):
    ...
    if tags is None:
        tags = ['education', 'language learning', 'telugu', 'funny']
...
def main():
    ...
    title = "Learn Telugu Daily"
    description = "#shorts #learntelugu #telugu #language #education"
    tags = ['telugu', 'education', 'language learning']
